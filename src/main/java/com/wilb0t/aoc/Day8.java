...
public class Day8 {
  ...
  public static int getAntiNodeCount(Input input) {
    ...
    return (int)
        coords.stream().filter(anode -> isValidCoord(anode, input.rows, input.cols)).count();
  }

  public static int getAntiNodeHarmCount(Input input) {
    var coords = new HashSet<Coord>();
    for (var entry : input.antennas.entrySet()) {
      var freqCoords = entry.getValue();
      for (var fidx = 0; fidx < freqCoords.size() - 1; fidx++) {
        ...
        for (var sidx = fidx + 1; sidx < freqCoords.size(); sidx++) {
          ...
          var fanode = new Coord(first.r - rdif, first.c - cdif);
          var sanode = new Coord(second.r + rdif, second.c + cdif);
          while (isValidCoord(fanode, input.rows, input.cols)) {
            coords.add(fanode);
            fanode = new Coord(fanode.r - rdif, fanode.c - cdif);
          }
          while (isValidCoord(sanode, input.rows, input.cols)) {
            coords.add(sanode);
            sanode = new Coord(sanode.r + rdif, sanode.c + cdif);
          }
        }
      }
      if (freqCoords.size() > 1) {
        coords.addAll(freqCoords);
      }
    }
